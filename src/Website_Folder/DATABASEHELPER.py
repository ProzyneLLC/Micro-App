...
def createConnCur():
    ...
    conn = mysql.connector.connect(
        host = 'test-mysqldb-fs.mysql.database.azure.com',
        user = 'test_admin',
        password = 'test_not_so_secure_Password_123',
        port = '3306',
        database = 'linkd'
        )
    # print('Connected to database')
...
def CreateUser(conn, cur, email, username, password):
    try:
        qr = """INSERT INTO users (email, username, password) VALUES(%s,%s,%s)
        ;
        """
...
def addLink(conn, cur, userid, linkitem, linktitle,linkcat, linkicon):
    # try:
        qr = """INSERT INTO links (userid, linkitem, linktitle, category, linkicon) VALUES(%s,%s,%s,%s,%s)
        ;
        """
...
def userFollow(conn, cur, followerid, followedid):
    try:
        qr = """INSERT INTO follow (followerid, followedid) VALUES(%s,%s)
        ;
        """
...
def addURL(conn, cur, userid, URLpp, URLbg):
    # try:
        qr = """INSERT INTO picture (userid, urlpp, urlbg) VALUES(%s,%s,%s)
        ;
        """
